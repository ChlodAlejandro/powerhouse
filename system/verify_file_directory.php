...
class FileDirectoryVerifier
{
    ...
    public static function verify() {
        if (file_exists(POWERHOUSE_DIR_FILES)) {
            if (!is_writable(POWERHOUSE_DIR_FILES)) {
            ...
            if ($mkdirOK) {
                ...
                if ($htaccessOK !== false) {
                    self::$ok = true;
                    self::$issue = new Exception("The directory HTACCESS file failed to write. This opens up the folder to intruders. Please allow Powerhouse to write on the files directory (\"" . POWERHOUSE_DIR_FILES . "\").");
                }
            } else {
